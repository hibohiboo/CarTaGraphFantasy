...
import { deriveArchetype, ARCHETYPE_LABEL, type Character, type Recruitment } from '@cartagraph/domain';
...
function fitOf(rc: Recruitment, chars: Character[]) {
  if (rc.prerequisiteTags.length === 0) return { fit: 'good' as const, label: '多くのPCが参加できます' };
...
export function SessionBrowsePage() {
  ...
  return (
    <>
      <PageHeader title="参加できるセッション" crumb="募集中のシナリオ。前提を満たさなくても応募できる（最終判断はGM）。自分のPCのほか、他PLのPCを借りて応募することもできる。" />
...
function RecruitCard({ rc, chars, myId }: { rc: Recruitment; chars: Character[]; myId?: string }) {
  ...
  const applied = rc.applicants.some((a) => chars.some((c) => c.id === a.characterId && c.ownerId === myId));
  ...
  return (
    <article className={s.recruit}>
      ...
      <div className={s.recruitMeta}>
        <span>
          想定人数 <strong>{rc.partySize.min}〜{rc.partySize.max}人</strong>（応募 {rc.applicants.length}/{rc.capacity}）
        </span>
        <span>
          空間モデル <strong>{rc.spaceModel === '2d' ? '2次元' : rc.spaceModel === '1d' ? '1次元' : '戦闘なし'}</strong>
        </span>
        <span>
      ...
      <div className={s.recruitApply}>
        {applied ? (
          ...
          <>
            <select aria-label="参加させるPC" value={characterId} onChange={(e) => setCharacterId(e.target.value)}>
              {chars.map((c) => (
            ...
            <Button variant="ink" block disabled={apply.isPending || !characterId} onClick={() => apply.mutate({ recruitmentId: rc.id, characterId })}>
              {apply.isPending ? '応募中…' : '応募する'}
            </Button>
            {apply.error && <ErrorNote error={apply.error} />}
            {fit.fit === 'partial' && <p className={s.recruitNote}>前提タグ不足ですが応募自体は可能です。</p>}
          </>
        )}
      </div>
    </article>
