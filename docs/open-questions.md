...
- **シーンカードの「目的」「終了条件」という属性** — 閲覧サイトのセッション試作（シーン構築・シーン進行画面）を作る中で出てきた案。[選択肢カード＝イベントカード](cartagraph/play-and-field.md#カードをプレイした結果として何が変化するのか決着)は「発火条件・効果・GM向け補足」の3要素構造と決着しているが、シーン単位で「目的」「終了条件」を持たせるかどうかは未検討。持たせる場合、既存の3要素構造や[終了条件（セッションの終了条件）](cartagraph/party-and-session.md#非同期セッションの進行決着)と役割が重複しないかも要確認。2026-09-23、ノード間の移動は選択肢カードの効果[「次のシーンへ進む」](cartagraph/play-and-field.md#次のシーンへ進む決着)として決着したが、シーン側の終了条件による遷移は引き続き未検討。アプリの型にある仮フィールド（`DeckNode.objective`・`endCondition`）と、選択肢カードの遷移先（`CardDef.nextNodeId`）の関係もあわせて整理する。
- **自動戦闘（仮ルール）の正式化** — GM不在のソロプレイ向けに、[自動戦闘（仮ルール）](cartagraph/auto-combat.md)を導入した（2026-09-23）。優先順位リスト、ラウンド上限、敗北時の再挑戦と「再挑戦の記憶」、軽量モードでのカウント制の自動実行、ソロ開始時の初期装備を、プレイテストを経て正式なルールにするかどうかは未検討。
- **カウント制の1ラウンドの行動回数と数値バランスの両立** — [数値バランスの相場観](cartagraph/balance.md)の「基本攻撃でHPの2〜3割」と「3〜5ラウンドで決着」は、カウント制で1ラウンドに何回行動できるか（行動値とコストの比）が決まらないと同時に満たせない。例えば行動値10〜12・コスト3では1ラウンドに3〜4回行動でき、1〜2ラウンドで決着してしまう。自動戦闘の検討（2026-09-23）で判明した。
