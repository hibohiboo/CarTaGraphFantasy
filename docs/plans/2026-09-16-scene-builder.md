...
**回答（2026-09-16）：選択肢1（推奨案）で進めることをユーザーが確認した。** これを受けて実装に着手した。

## 実装後のAIレビューで見つかった追加対応

design-reviewer／edge-case-reviewer／spec-reviewerの3並列レビュー（手順6）で、プランに書いていなかった問題が見つかった。ゲームデザイン上の判断ではなく明確な実装の誤り・欠落のため、人間に確認せずそのまま修正した。

- **P0：`kind === 'scene'`以外のDeckNodeもURL直打ちで編集できてしまう** — `CreatorSceneEditPage`のシーン検索に`kind`の絞り込みが無く、独立プールノード（`d-npc`等）や導入・結末ノードも編集できた。プラン自身の「独立プールノードは今回触らない」というスコープ外事項に反するため、`kind === 'scene'`でない場合はシーンが見つからない扱いにした
- **P0：エネミーカードに裏表トグルを許してしまっていた** — `docs/cartagraph/card-face-back.md`「エネミーカード｜常に公開＋状態タグ｜…裏表そのものは使わない」と矛盾していた。エネミーカードには裏表トグルを出さないよう修正した
- **P0：画像のlocalStorage書き込みでQuotaExceededError等が起きるとキャッチされず、シーン名等の未保存の編集ごと失われる** — 保存処理を`try/catch`で囲み、画像の書き込みに失敗してもシナリオ本体の保存は続行し、失敗を画面に表示するようにした
- **P1：カード削除・ロケーション差し替え時に古い画像がlocalStorageに残り続ける** — 削除・差し替え時に対応する`localStorage`エントリも削除するようにした
- **P1：画像を保存した後も「未保存」表示が消えない** — 表示条件を「編集内容に未保存の変更がある（`dirty`）」に連動させた
- **P1：画像読み込み失敗（`FileReader`のエラー）が無反応だった** — エラー時に画面へ表示するようにした
- **P1：カード追加フォームをキャンセルしても入力値が残る** — 開く・キャンセル時に入力欄をリセットするようにした
- **P2：同一ミリ秒内の連続操作でID衝突しうる** — IDに乱数を足して衝突しにくくした
- **P2：ロケーションのlocalStorage画像がプレビューに反映されていなかった** — 反映されるようにした
- **P2：同じファイルを選び直してもonChangeが発火しない** — file inputの値をリセットするようにした
- **テスト網羅の指摘**：「画像サイズが大きい場合の警告」「カードが0枚のシーンで一覧が空表示になること」「空白のみの名前が拒否されること（カード名・ロケーション名の両方）」のテストが計画にありながら実装から漏れていたため追加した
- **参考（対応不要）**：テスト基盤（`apps/web/src/test/setup.ts`）にNode 24の実験的`localStorage`グローバルとの衝突を避ける簡易実装を追加した。プランに書いていなかったが、`cardImageStorage`をテスト可能にするための技術的な必要事項であり、振る舞い・スコープへの影響はない
